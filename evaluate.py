...
def input_collate_fn(img, queries, queries_mask, imu_data):
    # this function takes (unprocessed) input data, and should return formatted input that can be fed directly into your model, e.g.
    # outputs = model(**input_collate_fn(img, queries, queries_mask, imu_data))
    return {"img": img, "queries": queries, "queries_mask": queries_mask, "imu_data": imu_data}
