...
on the validation set.  Run this after DETR training to find the optimal bias value,
then set BIAS in evaluate.py accordingly.
...
# ── collate with frozen QueryMLP ───────────────────────────────────────────────

def build_collate_fn(mlp_weights='query_mlp.pth'):
    from query_mlp import QueryMLP
    mlp = QueryMLP(input_dim=6, hidden_dim=128)
    mlp.load_state_dict(torch.load(mlp_weights, map_location='cpu'))
    mlp.eval()
    ...
    def collate_fn(batch):
        ...
        for img, q, imu in zip(imgs, queries, imus):
            imu_flat    = imu.flatten()
            ...
            with torch.no_grad():
                mlp_out = mlp(torch.tensor(mlp_input, dtype=torch.float32))
...
def load_model(checkpoint_path, device):
    ...
    model = DETR(backbone, transformer, input_dim_gt=4,
...
# ── collect raw logits in one pass ─────────────────────────────────────────────
...
def sweep(logits, has_label, ious):
    ...
    print(f"\n→  Set BIAS = {best_bias:.2f} in evaluate.py")
...
if __name__ == '__main__':
    ...
    loader  = DataLoader(dataset, batch_size=8,
                         ...
                         drop_last=False,
                         collate_fn=build_collate_fn(MLP_WEIGHTS),
                         num_workers=0)
